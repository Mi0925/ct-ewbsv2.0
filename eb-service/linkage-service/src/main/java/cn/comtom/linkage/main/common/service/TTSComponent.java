...
import cn.comtom.linkage.main.fegin.service.impl.SystemFeginServiceImpl;
...
public class TTSComponent {
    ...
    public Integer getProgramContentSpeechLength(String content) {
        try {
            //文转语音，计算播放需要的时长
            this.getInstance();
            String temp_file_path_ = systemFeginService.getByKey(Constants.TEMP_FILE_PATH);
            String dirPath=temp_file_path_+"/tts/";
    ...
    public String getTextToAudioFilePath(String content) {
        try {
            ...
            this.getInstance();
            String temp_file_path_ = systemFeginService.getByKey(Constants.TEMP_FILE_PATH);
            String dirPath=temp_file_path_+"audio/";
            File dir=new File(dirPath);
            if(!dir.exists())dir.mkdir();
            String fileName = UUIDGenerator.getUUID()+".mp3";
            String filePath=dirPath+fileName;
            boolean flag = this.synthesis(content, filePath);
            if(flag){
                File file=new File(filePath);
                return "/audio/".concat(fileName);
            }
        }catch (Exception e){
        	e.printStackTrace();
        }
    ...
    public static void main(String[] args) {
    	TTSComponent tts = new TTSComponent();
    	tts.API_KEY ="QzfRhA7iCpGiqQVwluYOyjVX";
    	tts.APP_ID="15977493";
    	tts.SECRET_KEY="ziNPW5GtFUM1CaXGbUWEmImVwwYfhh4j";
    	tts.getInstance();
    	tts.synthesis("百度你好，百度你好,百度你好，百度你好百度你好，百度你好百度你好，百度你好百度你好，百度你好百度你好，百度你好；","E:\\workTemp\\output.mp3");
    }
