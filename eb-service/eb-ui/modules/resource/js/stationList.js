$(function () {
    ...
    $("#jqGrid").jqGrid({
        url: baseURL+'/safeRest/ebr/ebrStation/page',
        datatype: "json",
        colModel: [
            { label: '台站/前端名称', name: 'ebrStName',  width: 100,align:"center"},
            { label: '台站/前端地址', name: 'address', width: 120 ,align:"center",
            ...
            {label: '资源编码', name: 'ebrStId', index: "ebrStId", width: 100, key: true},
            { label: '资源状态', name: 'stationState', width: 80,align:"center",
                formatter:function (cellvalue,options,row) {
        ...
        onSelectRow:function (rowid, status) {
            ...
            getStationDetail(rowData.ebrStId);
        },
    ...
    methods:{
        reload : function(){
            ...
            $("#jqGrid").jqGrid('setGridParam',{
                postData:{
                    "ebrStName":vm.req.ebrStName
                    ,"stationState":vm.req.stationState
                },
                page:page
            }).trigger("reloadGrid");
        },
...
function getStationDetail(ebrId) {
    if(isEmpty(ebrId)){
        return;
    }
    $.ajax({
        type: "get",
        url: baseURL+"/safeRest/ebr/ebrStation/"+ebrId,
        contentType: "application/json",
        success: function (r) {
            if (r.successful ) {
                vm.ebrInfo = r.data;
