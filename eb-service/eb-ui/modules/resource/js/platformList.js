$(function () {
    ...
    $("#jqGrid").jqGrid({
        url: baseURL+'safeRest/ebrPlatform/page?psType=01&queryType=2',
        datatype: "json",
        colModel: [
            { label: '平台名称', name: 'psEbrName', index: "psEbrName", width: 100,align:"center"},
            { label: '平台地址', name: 'psAddress', width: 120 ,align:"center",
            ...
            {label: '资源编码', name: 'psEbrId', index: "psEbrId", width: 100, key: true},
            { label: '资源状态', name: 'psState', width: 80,align:"center",
                formatter:function (cellvalue,options,row) {
                    return matchAndGetDictValue(vm.pageEnum,cellvalue,'RES_STATUS_DICT');
            }},
            { label: '汇报时间', name: 'updateTime', width: 100 ,align:"center"},
            { label: '联系人', name: 'contact', width: 50 ,align:"center"},
        ...
        onSelectRow:function (rowid, status) {
            ...
            getPlatformDetail(rowData.psEbrId);
        },
...
var dictGroupCodes = "RES_LEVEL_DICT,EBR_TYPE,RES_STATUS_DICT,EBR_BROADCAST_SUB_TYPE,EBR_PLATFORM_SUB_TYPE,SUB_RES_TYPE_DICT";
    ...
    methods:{
        reload : function(){
            ...
            $("#jqGrid").jqGrid('setGridParam',{
                postData:{
                    "psEbrName":vm.req.psEbrName
                    ,"psEbrId":vm.req.psEbrId
                    ,"psState":vm.req.bsState
                },
                page:page
            }).trigger("reloadGrid");
        },
...
function getPlatformDetail(ebrId) {
    if(isEmpty(ebrId)){
        return;
    }
    $.ajax({
