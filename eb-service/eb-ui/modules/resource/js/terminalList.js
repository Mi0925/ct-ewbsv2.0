$(function () {
    ...
    $("#jqGrid").jqGrid({
        url: baseURL+'/safeRest/ebr/terminal/page?queryType=2',
        datatype: "json",
        colModel: [
            { label: '终端名称', name: 'terminalEbrName', index: "terminalEbrName", width: 100,align:"center"},
            {label: '资源编码', name: 'terminalEbrId', index: "terminalEbrId", width: 100},
            { label: '资源状态', name: 'terminalState', width: 80,align:"center",
            ...
            { label: '经度', name: 'longitude', width: 60 ,align:"center"},
            { label: '纬度', name: 'latitude', width: 60 ,align:"center"}
        ],
        viewrecords: true,
        rowNum: 10,
        rowList : [10,30,50],
        ...
        onSelectRow:function (rowid, status) {
            ...
            getEbrTerminalDetail(rowData.terminalEbrId);
        },
...
var dictGroupCodes = "EBR_TYPE,RES_STATUS_DICT";
...
function getEbrTerminalDetail(ebrId) {
    if(isEmpty(ebrId)){
        return;
    }
    $.ajax({
        type: "get",
        async:false,
        url: baseURL+"safeRest/ebr/terminal/"+ebrId,
        contentType: "application/json",
        success: function (r) {
            if (r.successful ) {
                vm.ebrInfo = r.data;
