...
var dictGroupCodes = "PROGRAM_STATE_DICT,PROGRAM_TYPE_DICT,PROGRAM_CONTENT_TYPE,YANLIAN_STRATEGY_TYPE,PROGRAM_LEVEL_DICT,AUDIT_STATUS_DICT,MSG_LANG_DICT,WEEK_MASK_DICT,PROGRAM_STRATEGY_TYPE";
...
var vm = new Vue({
    ...
    data: {
       ...
       // programInfo:{},
        ...
        selectedAreaCodeArr:[],
        timeStyle:'1',
        timeAddClass:"program-time-add-btn",
        timeAddDisabled:false,
        programId:''
    ...
    mounted: function(){
       ...
       // this.getProgramInfo('52553a18da444ed8842674b161720484');
      //  this.getProgramInfo('');
        ...
        saveAndCommit:function(){
            ...
            if(vm.commitBtnEnable){
	            ...
	            $.ajax({
	                ...
	                success: function (r) {
	                    if (r.successful ) {
	                        window.location.href = 'programAudit.html';
	                    } else {
	                        layer.msg(r.msg);
	                    }
	                }
              	 ...
              	 layer.msg("节目提交中，请勿重复点击。请稍候。。。");
             }
        },
        initLayDate: function () {
        	var thisCTime = new Date().Format("yyyy-MM-dd hh:mm:ss").split(" ")[1];
        	var thisCDate = new Date().Format("yyyy-MM-dd");
        	var minTime = "00:00:00";
        	if(thisCDate == vm.programInfo.programStrategy.playTime){
        		minTime = thisCTime;
        	}
...
function initSelectTime(curDate){
	$("#startTime").replaceWith('<input id="startTime"  placeholder="开始时间" readonly  class="form-control eb-input-text" >');
	$("#overTime").replaceWith('<input id="overTime"  readonly placeholder="结束时间"  class="form-control eb-input-text" >');
	var minTime = "00:00:00";
	if(new Date().Format("yyyy-MM-dd")==curDate){
		minTime = new Date().Format("yyyy-MM-dd hh:mm:ss").split(" ")[1];
	}
    var startTimeRender= laydate.render({
        elem: '#startTime'
        ,type: 'time'
...
function next(flowState) {
	//校验
	if(flowState==2 && checkBaseInfo()){
		return;
	}else if(flowState==3 && checkAreaInfo()){
		return;
	}else if(flowState==4 && checkContentInfo()){
		return;
	}
...
function checkBaseInfo(){
	if(isEmpty(vm.programInfo.programName)){
		alert("节目名称不能为空！");
		return true;
	}
	if(isEmpty(vm.programInfo.programLevel) && vm.programInfo.programType==1){
		alert("请选择节目级别！");
		return true;
	}
	if(isEmpty(vm.programInfo.languageCode)){
		alert("请选择节目语种！");
		return true;
	}
	if(isEmpty(vm.programInfo.ebmEventType) && vm.programInfo.programType==1){
		alert("请选择事件分类！");
		return true;
	}
	return false;
...
//校验区域信息
function checkAreaInfo(){
	if(isEmpty(vm.areaNames)){
		alert("请选择区域！");
		return true;
	}
	return false;
...
function checkContentInfo(){
	if(isEmpty(vm.programInfo.contentType)){
		alert("请选择节目类型！");
		return true;
	}
	if((vm.programInfo.contentType==1 || vm.programInfo.contentType==4) && isEmpty(vm.programInfo.programContent.content)){
		alert("请输入文本内容！");
		return true;
	}
	if((vm.programInfo.contentType==2) && (vm.programInfo.filesList == null || vm.programInfo.filesList.length == 0)){
		alert("请上传文件！");
		return true;
	}
	if((vm.programInfo.contentType==3) && (vm.programInfo.filesList == null || vm.programInfo.filesList.length == 0)){
		alert("请选择曲目！");
		return true;
	}
	return false;
...
function checkStrategy(){
	var programType = vm.programInfo.programStrategy.programType;
	if(programType=='1'){
		vm.programInfo.programStrategy.strategyType = '1';
	}
	var psty = vm.programInfo.programStrategy;
	if(isEmpty(psty)||isEmpty(psty.strategyType)){
		alert("请选择节目策略！");
		return true;
	}
	if(psty.strategyType=='1'){
		if(isEmpty(vm.programInfo.programStrategy.playTime)){
			alert("请选择播放日期！");
			return true;
		}
	}
	if(psty.strategyType=='2' || psty.strategyType=='3'){
		if(isEmpty(vm.programInfo.programStrategy.startTime)){
			alert("请选择有效开始日期！");
			return true;
		}
		if(isEmpty(vm.programInfo.programStrategy.overTime)){
			alert("请选择有效结束日期！");
			return true;
		}
		if(vm.programInfo.programStrategy.startTime>vm.programInfo.programStrategy.overTime){
			alert("结束日期必须大于开始日期！");
			return true;
		}
	}
	if(psty.strategyType=='3'){
		if(isEmpty(psty.weekMask) || psty==0){
			alert("请选择星期！");
			return true;
		}
	}
	if(isEmpty(vm.programInfo.programStrategy.timeList)){
		alert("请添加时间区间！");
		return true;
	}
	if($("#durationTime").val() && $("#durationTime").val().length>2){
		alert("播发次数不超过100次！");
		return true;
	}
	return false;
...
function addProgramTime(obj) {
    ...
    if(vm.timeStyle == 1){
        startTime = $("#startTime").val();
        overTime = $("#overTime").val();
        if(isEmpty(startTime) || isEmpty(overTime)){
        	return;
...
function programTypeChange(thisele){
    var ptype = $(thisele).val();
    if(ptype=='2'||ptype=='3'){
    	vm.programInfo.programContent.content = '';
    }
}
function initJqgrid() {
    ...
    $("#jqGrid").jqGrid({
        ...
        onSelectRow:function (rowid, status) {
            ...
            var fileData = {fileId:rowData.id,fileName:rowData.libName}
            if(!status){
                //取消选中
...
function treeNodeChecked(event, treeId, treeNode) {
    ...
    //childrenDisabled(ztree,treeNode);
...
function initProgramTimeAddBtn() {
    if(vm.programInfo.programStrategy.strategyType == 1 && vm.programInfo.programStrategy.timeList.length > 0){
        vm.timeAddClass = "program-add-disabled";
        vm.timeAddDisabled = true;
    }
}
