package cn.comtom.reso.main.config;

import cn.comtom.reso.fw.FieldValidErrorException;
import cn.comtom.reso.fw.FieldValidError;
...
public class FieldValidBindResultAspect {
    ...
    @Pointcut(value = "execution(* cn.comtom.reso..*.controller..*Controller.*(..,org.springframework.validation.BindingResult,..))")
		...
		for(int i=0 ;i<args.length;i++){
		    ...
		    if(obj instanceof BindingResult){
                ...
                if (bResult.hasErrors()) {
                    throw fieldValidErrorResponse(bResult);
                }
                break;
            }
        }
